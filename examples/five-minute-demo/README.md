...
```bash
COMPOSER_DISABLE_NETWORK=1 COMPOSER_ROOT_VERSION=1.0.0 vendor/bin/upgrade-intel analyze --path=/path/to/php-upgrade-preflight/examples/five-minute-demo/target --from-php=8.1 --target=laravel/framework:^13.0 --target-php=8.3 --without-extension=ext-preflight-stage --framework=laravel --format=json --output=/tmp/php-upgrade-preflight-demo.json
```
...
The checked-in [JSON report](reports/laravel-10-to-13.json) is canonical. The [Markdown report](reports/laravel-10-to-13.md) is its human-readable projection. Both were generated by the analyzer against this target with Composer network access disabled; neither contains debug output.
...
COMPOSER_DISABLE_NETWORK=1 COMPOSER_ROOT_VERSION=1.0.0 vendor/bin/upgrade-intel analyze --path="$target" --from-php=8.1 --target=laravel/framework:^13.0 --target-php=8.3 --without-extension=ext-preflight-stage --framework=laravel --format=json --output=/tmp/php-upgrade-preflight-demo.json
