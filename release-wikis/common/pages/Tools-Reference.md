...
**Input and output.** The only input is a version without the `v` prefix. Success prints a confirmation; each inconsistency is printed as an `ERROR:` line. `v0.3.1`, `0.2.9`, and incomplete versions are rejected by the current release-series policy.
...
bash tools/release-distribution.sh --tag v0.3.1 --work /tmp/php-upgrade-preflight-dist
```
