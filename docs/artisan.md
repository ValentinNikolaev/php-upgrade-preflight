...
Artisan must boot before it can run the command. The published v0.3.1 adapter is tested as host-installable with Laravel 8–13. Use the external CLI for Laravel 7, when the current PHP interpreter cannot boot the application, when service providers are broken, or when installing the adapter would disturb the dependency graph.
