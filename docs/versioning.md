...
The active v0.3 development line is a new `0.MINOR` release. It introduces schema `0.8`, an optional stage-target provider, productionized sequential Composer results, and schema `1.0` partial or complete target-platform profiles. Historical v0.2 schemas and signed compatibility artifacts remain immutable, and every intentional migration is documented before v0.3 is released.
