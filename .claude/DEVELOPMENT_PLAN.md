...
- [ ] Verify PHP syntax, autoloading, dependency constraints, CLI startup, and Laravel provider startup on a machine with PHP and Composer.
...
- [ ] Choose PHPUnit or Pest with versions compatible with PHP 8.0.
- [ ] Add root scripts for tests, static analysis, and coding-style checks.
- [ ] Add fixture helpers that snapshot every original file before analysis and assert byte-for-byte immutability afterward.
- [ ] Add unit-test structure for core, CLI, and Laravel packages.
...
- [ ] Run `composer prohibits` or `why-not` in the temp workspace after failed target resolution when it adds diagnostic value.
...
- [ ] Test added, removed, upgraded, downgraded, source-ref, and dist-ref changes.
...
1. Make PHP and Composer available in the development shell.
