...
The separate compatibility workflow now performs clean normal and lowest-dependency consumer installs for core and CLI on PHP 8.0, Laravel 8/9 on PHP 8.0, Laravel 10 on PHP 8.1, and the declared Laravel 11/12 host integrations on PHP 8.2. The release workflow runs version-consistency checks, the deterministic gate, that compatibility workflow, and fresh-clone read-only audits on Windows and Linux before building checksummed Composer archives. Its artifact consumer installs the CLI and Laravel archives together, requires explicit Laravel adapter selection, verifies Laravel findings, analyzes a copied PHP 7.4 fixture from outside the target, and proves the target's recursive digest is unchanged. Tag-triggered runs require an annotated, GitHub-verified signature and a commit on the approved release line before publishing the GitHub release: `0.1.x` for the maintenance series and `main` for later approved series. Distribution-repository splits, signed tags, and Packagist synchronization remain maintainer operations.
