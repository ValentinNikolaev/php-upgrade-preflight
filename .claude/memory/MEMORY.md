...
- PHP Upgrade Preflight is a source-available public beta, not an Open Source project. PolyForm Noncommercial 1.0.0 permits free noncommercial use; commercial use requires a separate license from Valentin Nikolaev. Public beta is not a production-readiness certification, and reports never guarantee application runtime compatibility or deployment success.
- Commercial-license requests use the public Google Form linked from the README, LICENSE preamble, project-status page, and issue-template configuration. External code contributions are temporarily not accepted until the copyright holder adopts a legally reviewed contributor license agreement or another suitable inbound license grant. Documentation-only contributions, bug reports, private security reports, and product feedback remain welcome.
...
- The signed v0.2.1 release was cut from `main` under the former `0.2.x`-only gate. That historical policy is frozen with the release evidence; current development policy is the v0.3 line below.
- Active development is now `0.3.x-dev` on `main`, with tool metadata `0.3.0-dev` and strict report schema `0.8`. The signed v0.2.1 surface is frozen under `tests/fixtures/contracts/v0.2.1`, and v0.2.x patch releases are cut only from the published `0.2.x` maintenance branch. Live release-policy tests are separate from immutable v0.2 compatibility tests.
- Milestone 0 establishes a bounded staged-analysis vertical slice for exact Laravel 10→13 requests. The optional `FrameworkStageTargetProvider` supplies adjacent 10→11→12→13 targets without changing required v0.2 adapter interfaces; analyzer-owned Composer attempts carry the selected candidate manifest and lock forward, expose deterministic state fingerprints and multiple-blocker lifecycles, and never mutate the input fixture. The v0.3 contract and schema migration are frozen in `tests/fixtures/contracts/v0.3.json` and `docs/v0.3-contract.md`.
